...
> GM GRADE 7.25/10 - Some good writing and observations on variability and study design, but there are some issues:  code bugs (undefined `x_pop` in bootstrap section, incorrect relative error join, incomplete prompts), and several conceptual/language inaccuracies around bootstrap purpose and SE interpretation.
...
> *The GPP has the largest population SD, but not by much. This implies that the GPP is more variable than the TA.*
...
> GM: -0.5 The prompt is left blank.
...
n_grid <- c(50, 150, 300, 500)   # e.g., c(25, 100, 300, 1000)
...
summary_relerr <- sim_relerr %>%
  ...
  summarise(
    ...
    p90 = quantile(rel_error_pct, 0.90)
  )

p_relerr <- ggplot(summary_relerr, aes(x = n, y = median)) + 
  geom_point() +
  geom_line() +
  labs( title = "Relative Error decreases with n")
p_relerr
```
...
> *There is more improvement in the initial inclination of n densities compared to latter n densities. This accuracy shows diminishing returns because as more replications exist, the sample becomes closer and closer to the median. Just because of the law of randomness, and general data variability, the high-replicated sample data will never 100% match the original. I think of it like getting a 98% on a small assignment in class- you will never be able to get your grade exactly to the perfect 100% it was.*

> GM: The pattern is described reasonably well. Be careful with wording: increasing n reduces sampling variability / error of the estimate.
...
> *The se_theory and the se_simulated are not exactly close because the sample size is only 30. If the sample size (n_samp) was increased, as well a the reps, the se_theory and se_simulated would be closer to each other in value. This would showcase that the se_sim was calculated to a proper extent to match se_theory*

> GM: -0.5 There are issues in the SE comparison code and interpretation. Increasing `n_samp` reduces the SE (narrows the distribution). Increasing `reps` makes the simulation smoother but does not change the true SE. OVerall, the simulatuated and theoretical SE should be and are similar here.
...
samp <- tibble(x = sample(df[[target_var]], size = n_samp, replace = FALSE))
...
tibble(boot_means = boot_means) %>%
  ...
  geom_histogram(bins = 30, fill = "maroon") +
  geom_vline(xintercept = mean(samp$x), linetype = "dashed", linewidth = 0.75) +
  labs(
...
> *The bootstrap is attempting to approximate the population mean using a smaller, replicated sample. This sampling distribution is more bell shaped due to the high number of replications, being B=1000. Bootstrap can be especially useful in EaES when attempting to gauge a population estimate when you don't have access to enough samples. In this way, you can replicate the samples that you already have to derive something that is closer to the true mean and has a lower standard deviation overall.*

> GM: Yes, the bootstrap tries to approximate the sampling distribution of the mean using only one observed sample. This is especially useful in EAES when we cannot repeatedly sample the full population.
...
tibble(boot_mean = boot_means_random) %>%
  ...
  geom_histogram(bins = 20, alpha = 0.85, fill = "steelblue") +
  ...
  labs(
    ...
    subtitle = "Dashed = sample mean (i); Solid = population mean (ii)"
  )
```
...
> *The bootstrap distribution is centered near the sample mean, not as much the population mean. The bootstrap SD represents the standard deviation away from the core sample mean.The population mean is not centered in the histogram range.*
...
tibble(boot_mean = boot_means_summer) %>%
  ggplot(aes(x = boot_mean)) +
  geom_histogram(bins = 17, alpha = 0.85, fill = "steelblue") +
  geom_vline(xintercept = mean(swc_samp_summer), linetype = "dashed", linewidth = 1.1) +
  geom_vline(xintercept = pop_mean_swc, linetype = "dotted", linewidth = 1.1) +
  theme_classic(base_size = 18) +
  labs(
    x = "Bootstrap mean (swc)",
    y = "Count",
...
> *Yes, the bootstrap distribution looks confident and narrow even though its centered far from the population mean. This is because it sampled from a population only in the summer months, and didn't even consider the opposing winter months to form a bootstrap means. It is confidently marking the mean of the summer months, not the overall population mean. This distribution is precise to the summer samples provided but inaccurate to the population mean. This is introducing bias, or systematic error since deliberately sampling from summer months can skew the data towards one end of the population range.*
...
> *The random sample has the population mean put to the left, the summer-only sample has the population mean put extremely to the right, and the stratified sample has the population mean put gently to the right. The sampling design that is the most ACCURATE for the ANNUAL mean is the monthly-stratified sample, since each month is weighted equally for the bootstrap distribution sample. However, the summer-only sample is the most precise because the sampled data is more centralized towards lower swc's in the summer months, compared to the broad, rangy values you get when sampling the whole year. This teaches us that bootstrap resampling can be very biased depending on the methodology that you choose.*
---
...
>* Yes, the confidence interval contains the true population mean. This tells us that the uncertainty of the bootstrap estimate is aligned with the population mean in a well enough confidence interval to be applicable in science.
...
tibble(boot_mean = boot_means_random) %>%
  ggplot(aes(x = boot_mean)) +
  geom_histogram(bins = 17, alpha = 0.8, fill = "steelblue") +
  ...
  labs(
    ...
    y = "Count",
    title = "Bootstrap distribution (random sample)",
    subtitle = "Dashed = sample mean; Solid Red = 95% CI bars"
  )
```
...
>* The red vertical lines represent the range of values within 95% confidence of the mean. Any value outside of the space inbetween the red lines is NOT within 95% confidence
...
boot_coef <- replicate(B, {
  ...
  samp_rows <- new_df %>% 
    slice_sample(n = nrow(new_df), replace = TRUE)

  fit <- lm( samp_rows[[response_var]] ~ samp_rows[[predictor_var]]
)
  coef(fit)
...
>* No, the slope does not include 0. This implies that the predictor and response variable have a positive relationship within a 95% confidence interval. Since my predictor is atmospheric temperature (ta), and my response variable is Gross Primary Productivity (gpp), it would make sense that there is more biological productivity with increasing temperatures. 

> GM: Yes, and for best phrasing: "becaues the confidence interval does not include zero it supports a statistically significant relationships between x and y"
